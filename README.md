...
本文就目前几款主流小游戏设计引擎针对重物理题材的3D小游戏的渲染性能进行简要的测试对比。

测试方案是，在各个游戏引擎中设计出几乎相同的游戏场景，包括场景中的各种可能引发渲染代价的变量进行同步设定处理，然后将设计一个数量逐渐增大的方块自由掉落的实验进行渲染性能的测试。

这里需要提前说明一下，场景和基本的物理贴图、光照材质、摄像机位置两者几乎相同，但Demo的设计上及具体方法并未完全一样，由于本实验的目的就是假设在两款引擎的开发者按照每个引擎各自标准的设计方法去设计相同游戏内容的情况下的性能对比，所以引擎自身因为构建产生的渲染代价差异理应也在本次的性能对比的考察范围内。
...
PC：iMac 2015款 版本11.2.2 Core i7  / Chrome 版本 90.0.4430.93 / FireFox 版本 88.0.1 (64 位)

iPhone：iPhone11 Pro Max 版本14.4.2 A13 / Chrome 版本 87.0.4280.163 / 微信 版本 8.0.5 支持库 2.16.1

Android：小米10 版本10 内核4.19.81 骁龙865 / Chrome 版本 81.0.4044.117  / 微信 版本 8.0.3 支持库 2.16.1
...
数据列代表产生对应Cube时的渲染帧率以及对应的物理计算时差（**stepSimulation**），
...
| iPhone11 Pro Max Chrome - JS | 60 - 1            | -             | -           | -           | 60 - 8             | <u>59 - 11</u> | 26 - 27     |
| iPhone11 Pro Max WX - JS     | <u>20 - 38</u>    | 9 - 89        | 5 - 134     | 3 - 266     | 1 - 428            | -              | -           |
| iPhone11 Pro Max WX - WASM   | **<u>34 - 9</u>** | **21 - 18**   | **16 - 28** | **12 - 38** | **6 - 80**         | -              | -           |
| 小米10 Chrome - JS           | 60 - 1            | -             | -           | -           | 57 - 13            | 43 - 17        | 21 - 38     |
| 小米10 WX - JS               | 60 - 8            | <u>58 - 9</u> | 58 - 11     | 58 - 11     | 52 - 16            | 37 - 23        | 16 - 57     |
| 小米10 WX - WASM             | 60 - 8            | **60 - 8**    | **60 - 8**  | **60 - 9**  | <u>**57 - 14**</u> | **43 - 21**    | **23 - 42** |
...
| PC Firefox - JS                | 60 - 1            | <u>59 - 4</u> | 58 - 5         | 58 - 7             | 40 - 14            | 22 - 30            |
| PC Firefox - WASM              | 60 - 1            | **60 - 1**    | **-**          | **-**              | **<u>55 - 5</u>**  | **44 - 10**        |
| iPhone11 Pro Max Chrome - JS   | 60 - 1            | 60 - 1        | -              | -                  | 60 - 5             | 60 - 10            |
| iPhone11 Pro Max Chrome - WASM | 60 - 1            | 60 - 1        | -              | -                  | 60 - 4             | 60 - 8             |
| iPhone11 Pro Max WX - JS       | <u>46 - 28</u>    | 17 - 67       | 10 - 103       | 7 - 129            | 3 - 344            | -                  |
| iPhone11 Pro Max WX - WASM     | **60 - 4**        | **60 - 6**    | **60 - 11**    | <u>**45 - 15**</u> | **18 - 36**        | -                  |
| 小米10 Chrome - JS             | 60 - 1            | 60 - 2        | 60 - 6         | 60 - 10            | <u>58 - 14</u>     | 27 - 30            |
| 小米10 Chrome - WASM           | 60 - 1            | 60 - 2        | 60 - 3         | 60 - 5             | **60 - 8**         | <u>**45 - 15**</u> |
...
从上述的实验报告中，可以看出目前的两款游戏引擎的性能差异（Threejs作为对照组），其中Cocos Creator3.0与LayaBox在浏览器环境中（包括移动设备中原生浏览器）表现相对较好，场景内渲染元素在200个以内均能保持60FPS的稳定渲染，可适用于一般的小游戏作品。在微信小游戏环境中，本文采用iPhone11 Pro Max（2019年9月上市，A13处理器）与小米10（2020年2月上市，骁龙865处理器）两款进行作为对比，从原生浏览器环境中可以看出两款机型性能几乎无差别，但由于iOS的微信小游戏中是没有JIT的，因此iOS中无法表现出与浏览器内的性能效果，不过对比使用js与wasm版本的物理引擎下可以看出，使用wasm的物理引擎能够明显的提升运行性能，提高渲染帧率，且从物理计算耗时中也可以看到提升效果极其明显。对比两款游戏引擎而言，无论是iOS还是Android中，LayaBox均优于Cocos Creator3.0，目前Cocos Creator3.0对wasm模式刚刚支持，且不支持引擎分离模式发布，主包体积通常也会超过微信小游戏的发布限制，开发者需要自行分包处理，综合而言，目前LayaBox支持的更全面，性能更好。此外本文还增加了一组使用Threejs完成的对照实验，重点对照的是Cocos Creator3.0的实验数据，在Threejs的实验中，本文使用了与Cocos Creator3.0内完全相同的Ammo的js与wasm版本作为物理引擎，因此从结论上可以看出，Cocos Creator3.0的内部引擎对性能的消耗是造成低性能主要原因，Cocos Creator引擎因提供了类似Unity的设计模式，以及丰富的可视化设计工具，受到开发者的青睐，同样的，设计方便的代价是损失了一些游戏的运行性能，不过相信Cocos Creator未来会逐步优化好，保留现在有的设计优势的同时缩小性能上与其他引擎的差距。

总结：

1. LayaBox性能优于Cocos Creator3.0，

   LayaBox几乎与使用Three.js性能匹敌，

   因此在有大量元素场景中，推荐使用LayaBox设计3D的物理小游戏；

2. 使用wasm版本物理引擎将会比js版本的物理引擎提升性能，尤其在iOS的微信小游戏中，推荐使用wasm。
