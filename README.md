...
Threejs是渲染引擎，在本文中作为游戏引擎的实验数据参照，因此单独对Threejs在微信小游戏中的性能进行测试并记录。

| 设备-运行环境              | 50Cube (FPS - ms) | 100    | 150    | 200    | 400     | 800     |
| -------------------------- | ----------------- | ------ | ------ | ------ | ------- | ------- |
| iPhone11 Pro Max WX - JS   |                   |        |        |        |         |         |
| iPhone11 Pro Max WX - WASM |                   |        |        |        |         |         |
| 小米10 WX - JS             | 60 - 2            | 60 - 4 | 60 - 7 | 60 - 9 | 60 - 11 | 52 - 17 |
| 小米10 WX - WASM           |                   |        |        |        |         |         |
...
从上述的测试报告中，已经可以对比出两款游戏引擎的性能差异，我们重点观察在小于60FPS时，方块产生的数量变化时导致的性能差异。CocosCreator3.0对wasm模式才刚刚支持，且只能在微信小游戏环境中使用，不过从实验结果来看，CocosCreator3.0目前在各项数据表现而言均略微逊色于LayaBox，并且目前如果使用wasm模式，不支持引擎分离模式发布，其主包体积会超过微信小游戏的发布限制，需要开发者手动进行分包处理。在性能表现上使用wasm版本相比于js版本略微有所提升，但作为CocosCreator的实验性选项当前版本还是不建议开发者使用。从现状看LayaBox不仅支持的更全面，而且性能仍然优于CocosCreator3.0,。从其他运行环境中，LayaBox的JS版本物理引擎的性能也优于CocosCreator3.0。其中LayaBox的wasm版本的物理引擎在不同的运行环境中相对于JS版对性能优化上是有不同程度的提高。

总的来说，LayaBox引擎对性能的极致优化是有目共睹的，在3D游戏设计中目前LayaBox仍然是更胜一筹。不过相信CocosCreator对3D游戏的逐步布局以及打造的H5游戏一系列设计模式，未来也会逐渐缩小与前者的差距，变得更加专业。
